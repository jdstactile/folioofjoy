...
const EXPERIMENTS: Experiment[] = [
  {
    src: '/experiments/deinfluence.png',
    title: 'De-Influence Machine',
    url: 'https://deinfluence.me',
    caption: 'An impulse purchase prevention system — type in what you want to buy and let it talk you out of it.',
  },
  {
    src: '/experiments/notesforlazy.png',
    title: 'Notes for Lazy',
    url: 'https://notesforlazy.vercel.app/',
    caption: 'A minimalist markdown note app for people who want to write, not configure.',
  },
  {
    src: '/experiments/sneakers.png',
    title: 'White Sneaker Directory',
    url: 'https://white-sneaker-directory.vercel.app',
    caption: 'A curated catalogue of white sneakers because life is too short to scroll through everything else.',
  },
  {
    src: '/experiments/reading.png',
    title: 'What Am I Reading',
    url: 'https://whatamireading.vercel.app',
    caption: 'A reading tracker that shows your year in books — pages, titles, and a share card for the flex.',
  },
];
...
export function ExperimentsLink() {
  ...
  return (
    <>
          ...
          <span className="relative block w-44 h-32">
            {EXPERIMENTS.slice(0, 3).map((item, i) => (
              <img
                ...
                style={{
                  left: showPreview ? `${i * 10}px` : '20px',
                  top: showPreview ? `${i * -5}px` : '10px',
                  zIndex: 3 - i,
                  transform: showPreview ? `rotate(${(i - 1) * 5}deg) scale(1)` : 'rotate(0deg) scale(0.85)',
      ...
      {carouselMounted && (
        <div
          className={`fixed inset-0 z-[100] flex items-center justify-center transition-opacity duration-400 ease-out ${carouselVisible ? 'opacity-100' : 'opacity-0'}`}
          onClick={closeCarousel}
        >
          <div className={`absolute inset-0 bg-black md:bg-black/90 backdrop-blur-sm transition-all duration-400 ${carouselVisible ? 'opacity-100' : 'opacity-0'}`} />
          ...
          <div className="relative z-10 flex flex-col items-center gap-6 max-w-5xl w-full px-4 md:px-20" onClick={(e) => e.stopPropagation()}>
            ...
            <div key={captionKey} className="flex flex-col items-center gap-2 animate-caption-fade">
              ...
              <a
                href={EXPERIMENTS[currentIndex].url}
                target="_blank"
